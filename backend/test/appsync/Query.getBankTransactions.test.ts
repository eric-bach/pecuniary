...
const file = '../backend/appsync/build/Query.getBankTransactions.js';

const appsync = new AppSyncClient({ region: 'us-east-1' });

describe('Query.getBankTransactions', () => {
  it('returns all bank transactions', async () => {
    // Arrange
    const context = {
      arguments: {
        accountId: '123',
    ...
    expect(filterExpressionValues[':entity']).toEqual('bank-transaction');
  });
});
