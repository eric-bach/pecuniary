...
const file = '../backend/appsync/build/Query.getAccounts.js';

const appsync = new AppSyncClient({ region: 'us-east-1' });

describe('Query.getAccounts', () => {
  it('returns all accounts', async () => {
    ...
    expect(result.index).toEqual('userId-gsi');
    ...
    expect(filterExpressionValues[':entity']).toEqual('account');
  });
});
